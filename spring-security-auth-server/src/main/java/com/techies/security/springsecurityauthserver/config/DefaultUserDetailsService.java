...
public class DefaultUserDetailsService implements UserDetailsService {
	...
	private UserDetails mockUser(String username) {
		...
		// you can also use a prefix, @see
		// com.patternmatch.oauth2blog.config.AuthorizationServerConfig#CLIENT_SECRET
		UserDetails user = User.builder().username(username).password(userPass).authorities(getAuthority()).build();
